...
export function createStorage(storePath: string | null) {
  const [data, setData] = useMutative<Record<string, any> | undefined>(
    undefined
  );
    ...
    if (isTauri()) {
      (async () => {
        try {
          ...
          if (store === null)
            throw new Error(`invalid path ${storePath} for store`);
          fileStoreRef.current = store;
          const value =
            await fileStoreRef.current.get<Record<string, any>>('data');
          if (value === undefined) {
            const newObj = {};
        ...
        if ((err !== undefined && value === null) || Array.isArray(value)) {
          localforage.setItem(storePath, {}, (err, val) => {
            if (err !== null && err !== undefined) {
              return alert(
                'cannot store data, application will not work as intended'
              );
            }
            setData(val);
          });
  ...
  const setItem = useCallback(
    (key: string, newValueOrHandler: Dispatch<SetStateAction<any>>) => {
      if (loading) return;
        ...
        if (value !== prev) {
          if (isTauri()) {
            ...
            timeoutRef.current = window.setTimeout(
              () => localforage.setItem(storePath, data),
              SAVE_DELAY
            );
          }
        }

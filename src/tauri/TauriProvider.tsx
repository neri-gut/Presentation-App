  ...
  if (isTauri()) {
    ...
    useEffect(() => {
      ...
      setUsingCustomTitleBar(
        !isFullScreen && osType === 'windows' && WIN32_CUSTOM_TITLEBAR
      );
    }, [isFullScreen, osType]);
...
export async function getUserAppFiles() {
  ...
  for (const path of await readDirRecursively(
    APP_NAME,
    fs.BaseDirectory.AppData
  )) {
    const friendlyName = path.substring(appFolder.length + 1, path.length);
    if (EXTS.has(getExtension(path).toLowerCase()))
    ...
    if (entry.isDirectory) {
      files.push(
        ...(await readDirRecursively(
          await tauriPath.join(path, entry.name),
          baseDir
        ))
      );

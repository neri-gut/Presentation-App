...
export default function ExampleView() {
  ...
  const storeName = isTauri()
    ? join(fileSep, documents!, APP_NAME, 'example_view.dat')
    : 'example_view';
  ...
  return (
    <Stack>
      ...
      <Trans
        ...
        // optional stuff:
        default="FALLBACK if key does not exist. This template is from <0>github.com{{variable}}</0>"
        t={t}
      />

      {loading ? (
        <Text>Loading Tauri Store</Text>
      ) : (
        <>
          <TextInput
          ...
          />
          <Button onClick={() => revealItemInDir(storeName)}>
            Reveal store file in file directory
          </Button>
        </>
      )}
    </Stack>
